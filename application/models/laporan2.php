...
class Laporan2 extends CI_Model{
    ...
    public function datalist_service($id_divisi_lap, $tgl_awal, $tgl_akhir)
    {

        if($id_divisi_lap>0){
            
            $query = $this->db->query("SELECT A.status, DATE_FORMAT(A.tanggal, '%d-%m-%Y %H:%i:%s')as tgl_service, 
									A.skomputer, A.sparepart, A.id_kondisi, A.id_service, A.status,
                                   ...
                                   WHERE A.status IN (1,2,3,4,5,6) AND F.id_divisi=$id_divisi_lap AND (convert((DATE_FORMAT(A.tanggal, '%d-%m-%Y')), char)) BETWEEN '".$tgl_awal."' AND '".$tgl_akhir."'") ;
        }
    else{
            $query = $this->db->query("SELECT A.status, DATE_FORMAT(A.tanggal, '%d-%m-%Y %H:%i:%s')as tgl_service, 
									A.skomputer, A.sparepart, A.id_kondisi, A.id_service, A.status,
									...
									LEFT JOIN barang C ON C.id_barang = A.id_barang
									LEFT JOIN divisi F ON F.id_divisi = D.id_divisi
                                       WHERE A.status IN (1,2,3,4,5,6) AND (convert((DATE_FORMAT(A.tanggal, '%d-%m-%Y')), char)) BETWEEN '".$tgl_awal."' AND '".$tgl_akhir."'") ;
        }
